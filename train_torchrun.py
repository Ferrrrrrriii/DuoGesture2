...
def _load_hubert_stats(args, device):
    ...
    if not mean_path or not std_path or (not os.path.isfile(mean_path)) or (not os.path.isfile(std_path)):
        logger.warning("HuBERT mean/std paths are missing or files do not exist; skip injection and use model-side normalization")
        return None, None
...
def _inject_hubert_stats(model_wrapped, mean_t, std_t):
    ...
    if hasattr(mod, "set_hubert_dataset_stats"):
        ...
        logger.info("Injected HuBERT mean/std into model")
    else:
        logger.warning("Model does not implement set_hubert_dataset_stats; skip HuBERT mean/std injection")
    ...
    def __init__(self, args):
        ...
        if args.e_name is not None:
            ...
            if self.args.ddp:
                ...
                self.eval_copy = getattr(eval_model_module, args.e_name)(args).to(self.device)  # single-GPU eval copy
            else:
...
def main_worker(args):
    ...
    # Initialize distributed mode when DDP is enabled and world_size > 1.
    ...
    if getattr(args, "load_ckpt", None):
        ...
        if os.path.exists(load_ckpt):
            ...
            other_tools.load_checkpoints(trainer.model, load_ckpt, "duogesture_model")
            logger.info("Checkpoint loaded successfully")
        else:
                logger.warning(f"Checkpoint {load_ckpt} does not exist; skip loading")
