...
# Adjust this constant to your local model path when needed.
...
def get_word_sentence(
    audio_path: str,
    args,
    window: int = 64,                 # one window per 64 frames
    fps: Optional[float] = 30,        # default 30 fps
    total_frames: Optional[int] = None,
    model_size: str = LOCAL_WHISPER_MODEL_DIR,
    language: Optional[str] = None,
    batch_size: int = 1,              # default batch_size=1
    device: Optional[str] = None,
) -> Tuple[torch.LongTensor, List[List[str]]]:
    """
        Returns:
            in_word: torch.LongTensor, shape [1, frame]
      in_sentence: List[List[str]], 长度 = (frame - 4) // (window - 4)
                                     where the k-th sentence covers frame range
                                     [k*(window-4), k*(window-4)+window)
    """
    ...
    # 1) Flatten aligned words into a word-level list.
    words = []
    ...
    # 3) Select a token for each frame.
    ...
    # 4) Build in_sentence with a 64-frame window and 60-frame stride.
    stride = window - 4
...
def get_emo(audio_path, frame):
    # Extract emotion labels from audio chunks.
...
def get_max_score_label(wav_file, start_frame, end_frame, model, fps=30, target_sr=16000):
    """Extract an audio segment and return the top-scoring emotion label."""
