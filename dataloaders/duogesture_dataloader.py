...
def _resolve_legacy_data_path(path: str) -> str:
    if os.path.exists(path):
        return path
    if "duogesture" in path:
        legacy_path = path.replace("duogesture", "duogesture")
        if os.path.exists(legacy_path):
            logger.warning(f"Data path {path} not found; falling back to legacy path {legacy_path}")
            return legacy_path
    return path

class LMDBNPZDataset(torch.utils.data.Dataset):
    """
    Read an LMDB dataset where each sample is returned as
    {key: np.ndarray or object}.
    """
    def __init__(self, args, loader_type='train'):
        args.train_path = _resolve_legacy_data_path(args.train_path)
        args.test_path = _resolve_legacy_data_path(args.test_path)
...
class PickleDataset(torch.utils.data.Dataset):
    """
    Read a .pkl file whose content is a list of dictionaries.
    Loaded into memory once for simplicity; for very large datasets,
    this can be changed to chunked files.
    """
    ...
    def __getitem__(self, idx: int):
        ...
        # Convert object ndarrays for sentence/emo to Python lists.
        for k in ("sentence", "emo"):
            if k in sample and isinstance(sample[k], np.ndarray) and sample[k].dtype == object:
                sample[k] = sample[k].tolist()
        return sample