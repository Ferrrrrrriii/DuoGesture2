...
if __package__ is None or __package__ == "":
    # executed as a script: add project root (DuoGesture) into sys.path
...
def _lmdb_entries(lmdb_dir: str) -> int:
    """Return number of samples in LMDB; return 0 if missing/unreadable."""
    try:
...
class Processor:
    ...
    def __init__(self, device: torch.device, args):
        ...
        # --- Text encoder: TMR / MoCLIP / vanilla CLIP ---
    ...
    def process(self, dict_data: Dict[str, Any], mode: str = "train") -> Dict[str, np.ndarray]:
        """
        Process one sample and return a .npz-serializable numpy dict.
        Assumes dict_data includes at least:
            pose(axis-angle, [1,64,169]), trans, facial, audio, hubert,
            beat, word, sentence, emo, beta, id, ...
        """
        # rc = self.rc

        # ---- Load fields and move to device ----
        tar_pose_raw = dict_data["pose"].to(self.device)            # [B,N,169]
        tar_pose = tar_pose_raw[:, :, :165]                          # 55*3 axis-angle
        ...
        if mode == "train":
            ...
            # exit(0)
            feat_clip_text = self.encode_text(in_sentence, self.device)           # expected [B, D] or [B, 1, D]
        ...
        # ---- Split body parts + axis-angle -> matrix -> rot6d ----
        # jaw
        ...
        zq_face  = self.vq_model_face.map2zq(tar_pose_face)    # [B,6,1,16,256] (example shape)
        ...
        # Full 55-joint axis-angle -> rot6d
        ...
        # Build output dict (adjust keys as needed).
        out = {
            # raw/intermediate features (batch dim removed)
            ...
            # text encoding outputs (batch dim removed)
...
def pack_npz_bytes(arr_dict: Dict[str, np.ndarray]) -> bytes:
    ...
    with io.BytesIO() as bio:
        # Keep pickle disabled to avoid unintended object serialization.
...
def _to_np_squeezed(x):
    """
    Remove leading batch dim (=1) before saving and keep other dims unchanged.
    Only squeeze numeric arrays; object fields are handled separately.
    """
    a = _to_np(x)
...
def main():
    ...
    ap.add_argument("--dst_lmdb", default="./datasets/beat2_duogesture_train_eval/" ,help="Destination LMDB directory")
    ap.add_argument("--moclip_path", default=None, type=str, help="Path to TMR/MoCLIP text encoder checkpoint. If set, uses TMR instead of vanilla CLIP.")
    ap.add_argument("--distilbert_path", default="distilbert-base-uncased", type=str, help="HF model name or local path for DistilBERT (TMR backbone)")
    ap.add_argument("--clip_version", default="ViT-B/32", type=str, help="CLIP backbone architecture (ViT-B/32 or ViT-L/14)")
    ...
    # === Skip if target LMDB already exists and is not empty ===
    if os.path.isdir(args.dst_lmdb):
        n = _lmdb_entries(args.dst_lmdb)
        if n > 0:
            print(f"[Skip] LMDB already exists and is not empty: {args.dst_lmdb} ({n} entries).")
            return
