...
class Processor:
    ...
    def process(self, dict_data: Dict[str, Any], mode: str = "train") -> Dict[str, np.ndarray]:
        """
        Process one sample and return a pickle-serializable numpy dict.
        ...
        latent_all = torch.cat([tar_pose_6d, tar_trans, tar_contact], dim=-1)     # [B,N,337]
           ...
           # Build output dict (batch dim removed)
        out = {
...
# --------------------------------
# Pickle writer (replaces LMDB + NPZ flow)
# --------------------------------
def write_pickle(stream: Iterable[Dict[str, np.ndarray]], dst_pkl: str) -> int:
    """
    Write sample stream to a .pkl file as [sample_dict, sample_dict, ...].
    Note: this buffers all samples in memory before writing once.
    For large datasets, split output into chunks/files.
    """
    ...
    with open(dst_pkl, "wb") as f:
        # Use latest protocol for better large-array support.
...
def main():
    ...
    # Target pickle output path.
    ap.add_argument("--dst_pkl", default="./datasets/beat2_duogesture_test.pkl" ,help="Destination pickle file")
    ...
    args = ap.parse_args()
    # === Skip if target pickle already exists ===
    if os.path.isfile(args.dst_pkl):
        print(f"[Skip] Target file already exists: {args.dst_pkl}")
        return
    ...
    def sample_stream() -> Iterable[Dict[str, np.ndarray]]:
        for batch in tqdm(loader, desc="Processing"):
            # batch can be dict or list/tuple -> dict_data
