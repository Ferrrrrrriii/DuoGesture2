...
def parse_args():
    ...
    parser.add("--base_ckpt", default="weights/best_duogesture_base.bin", help="base ckpt path")
    ...
    parser.add("--train_path", default="./datasets/beat2_duogesture_train", type=str)
    parser.add("--test_path", default="./datasets/beat2_duogesture_test.pkl", type=str)
    ...
    parser.add("-c", "--config", default='./configs/duogesture_base.yaml', is_config_file=True)
...
#    parser.add("--gan_noise_size", default=0, type=int)
    ...
    # Latent loss gating mode: gt_mask (legacy), pred_gate, uniform
    parser.add("--latent_loss_mode", default="gt_mask", type=str)
    parser.add("--sem_boost", default=3.0, type=float)
    ...
    parser.add("--phys_upper_only", default=False, type=str2bool)
    ...
    parser.add("--phys_hands_lambda", default=1.0, type=float)
    parser.add("--phys_warmup_start", default=30, type=int)
    parser.add("--phys_warmup_end", default=80, type=int)
    # Option 1: fused latent jerk — penalise jerk of gate-blended latent (no decoder pass)
    parser.add("--phys_fused_latent", default=False, type=str2bool)
    parser.add("--phys_fused_lambda", default=0.10, type=float)
    # Gate-complementary physics: weight jerk by (1-ψ), gradient flows through ψ
    parser.add("--phys_gate_grad", default=False, type=str2bool)
    # Adaptive inertia: normalise CV residual by speed² (dynamic, motion-state-aware)
    parser.add("--phys_adaptive_inertia", default=False, type=str2bool)
    # Gate floor: ψ_eff = floor + (1-floor)*ψ — prevents ψ=0 from killing semantic signal
    parser.add("--gate_floor", default=0.0, type=float)
    # Option 3: gate temporal smoothing — penalise abrupt gate transitions
    parser.add("--gate_smooth_weight", default=0.0, type=float)
    # Directly penalise the mean gate value: loss += λ * mean(ψ)  → always-on gate is expensive
    parser.add("--gate_mean_weight", default=0.0, type=float)
